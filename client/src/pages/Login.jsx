...
const Login = () => {
    ...
    return (
        <div
            ...
            <div
                className='bg-slate-900 p-10 rounded-lg w-full sm:w-96 text-indigo-300 text-sm'>
                <h2
                    className='text-3xl font-semibold text-white text-center mb-3'>{state === 'Sign Up' ? 'Create Account' : 'Login'}</h2>
                <p
                    className='text-center text-sm mb-6'>{state === 'Sign Up' ? 'Create your Account' : 'Login to Your Account'}</p>
                ...
                {/* here we are just using a conditional if else case on two p tags based on the state */}
                {state === 'Sign Up' ?
                    (<p
                        className='text-gray-400 text-center text-xs mt-4'>
                        Already have an account?{' '} <span
                            onClick={() => setState('Login')}
                            className='text-blue-400 cursor-pointer underline'>Login</span>
                    </p>
                    )
                    : (
                        <p
